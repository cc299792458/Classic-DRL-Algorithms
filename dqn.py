...
class DQN():
    def __init__(self, gamma, epsilon, lr, input_dims, n_actions, batch_size,
                 max_mem_size=100_000, eps_min=0.01, eps_dec=5e-4):
        self.gamma = gamma
        ...
        # self.terminal_memory = np.zeros(self.max_mem_size, dtype=np.bool8)
        
    def store_transition(self, state, action, reward, state_, done):
        self.replaybuffer.store_transition(state=state, action=action, reward=reward, state_=state_, done=done)
    ...
    def learn(self):
        ...
        batch_index, state_batch, new_state_batch, reward_batch, terminal_batch, action_batch = self.replaybuffer.generate_batch(self.Q_eval.device)
